...
class EulerSolver {
    ...
    static void PrintResults(output results, std::vector<std::string> Xstring, std::vector<std::string> Ystring) {
        std::cout << "Modelling results:" << std::endl;
        std::cout << std::setw(10) << "t, s";
        for (auto x_name : Xstring) { 
            std::cout << std::setw(9) << x_name;
            if(x_name[0] == 'U') std::cout << ", V";
            else if(x_name[0] == 'I') std::cout << ", A"; 
        }
    ...
    if (!results.empty()) {
        ...
        for(size_t xi = 0; xi < X_end.getRows(); ++xi) {
            ...
            if(Xstring[xi][0] == 'U') std::cout << " V" << std::endl;
            else if(Xstring[xi][0] == 'I') std::cout << " A" << std::endl; 
        }
